...
// ── Analytics helpers ─────────────────────────
// Wraps logEvent so all tracking calls are in one place.
// Events show up in Firebase Console → Analytics → Events (may take 24h on free plan;
// use DebugView for real-time during development).
...
onAuthStateChanged(auth, async (firebaseUser) => {
  if (firebaseUser) {
    ...
    // setUserId lets Analytics tie all events to one user across sessions.
    // We use the Firebase uid (not username) to stay anonymous in Analytics.
    ...
    window.S.uid     = null;
    track('logout');
    // Clear analytics identity on sign-out
    try { setUserId(analytics, null); } catch(e) {}
    document.getElementById('screen-app').classList.add('hidden');
    document.getElementById('screen-auth').classList.remove('hidden');
