...
// ── In-memory DB mirror ───────────────────────
window.DB = { users:{}, problems:[], folders:[], assignments:[], posts:[], topics:[], homepage:{}, events:[] };
window.S  = {
...
async function loadSharedData() {
  const [probSnap, postSnap, assignSnap, folderSnap, topicSnap, hpSnap, evSnap, ratingSnap] = await Promise.all([
...
window.saveUserOnly = async function() {
  ...
  // Only write fields a student is allowed to change.
  // Never send isAdmin — the Firestore rule blocks it and it would be a security hole.
  const safe = {
    scores:            u.scores            || {},
    streak:            u.streak            || 0,
    assignSubmissions: u.assignSubmissions || {},
    notifPrefs:        u.notifPrefs        || {},
    attemptLog:        u.attemptLog        || [],
  };
  try {
...
window.renderUserMgmt = async function() {
  ...
  users.forEach(u => {
    ...
    row.innerHTML = `
      ...
      ${!isSelf
        ? `<button class="btn btn-sm" onclick="toggleAdmin('${u.uid}',${JSON.stringify(u.username)},${!u.isAdmin})">
             <i class="ti ${u.isAdmin ? 'ti-shield-off' : 'ti-shield-check'}"></i>
             ${u.isAdmin ? 'Remove' : 'Make admin'}
           </button>
           <button class="btn btn-sm btn-red" onclick="deleteUser('${u.uid}',${JSON.stringify(u.username)})">
