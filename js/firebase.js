...
import {
  ...
  collection, getDocs, deleteDoc, arrayUnion
...
window.saveDB = async function() {
  ...
  // Always save current user's profile
  const u = window.DB.users[window.S.user];
  if (u) {
    await setDoc(doc(db, 'users', uid), u, { merge: true });
  }

  // Save shared data (problems, posts, assignments, folders)
  // We write all items — Firestore merge means unchanged docs cost minimal reads
  const writes = [];
...
// Fetch all user docs fresh from Firestore (used by grade table)
