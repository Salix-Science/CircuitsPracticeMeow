...
// Cached variant values keyed by `${assignId}-${probId}`
if (!window._assignVals) window._assignVals = {};

// ── Render assignment list (student) ──────────
function renderStudentAssignments() {
  ...
  if (!visible.length) {
    wrap.innerHTML = '<div style="color:var(--text4);font-size:13px;padding:2rem;text-align:center">No assignments are open right now.</div>';
    return;
  }
...
function renderAssignProblems(assign, wrap) {
  ...
  assign.problems.forEach((ap, idx) => {
    // Assignments always access problems regardless of enabled state
    ...
    row.innerHTML = `
      ...
      ` : `
        <div class="feedback ${done.correct ? 'correct' : 'wrong'}" style="display:block">
          ${done.correct
            ? `✓ Correct · ${done.submitted} ${p.unit}`
            : `✗ Your answer: ${done.submitted} ${p.unit} · Expected: ${p.answer} ${p.unit}`}
        </div>`}`;
...
// ── Reshuffle a single assignment problem ─────
...
async function submitAssignProb(assignId, probId, varKey) {
  ...
  if (isNaN(raw)) {
    if (fb) { fb.textContent = 'Enter a number.'; fb.className = 'feedback wrong'; fb.style.display = 'block'; }
    return;
  }

  const submitted = raw * mult;
  const tol       = Math.abs(p.answer) * p.tol + 0.001;
  const correct   = Math.abs(submitted - p.answer) <= tol;
