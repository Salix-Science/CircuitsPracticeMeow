...
function resetForm(){
  ...
  document.getElementById('e-unit').value='V';document.getElementById('e-tol').value='2';document.getElementById('e-pts').value='10';
  document.getElementById('var-rows').innerHTML='';document.getElementById('img-preview-wrap').innerHTML='';
...
function addVar(type){
  ...
  const mins={R:'1',V:'5',I:'1',C:'1'},maxs={R:'10',V:'24',I:'10',C:'100'};
...
function renderVarRows(){
  ...
  window.S.editorVars.forEach((v,i)=>{
    const ul={R:'kΩ',V:'V',I:'mA',C:'μF'}[v.type]||'';
    const row=document.createElement('div');row.className='var-row';
    row.innerHTML=`<div><label>Name</label><input type="text" value="${v.name}" oninput="window.S.editorVars[${i}].name=this.value;renderVarInsertChips();previewFormula()"/></div>
      <div><label>Unit</label><input type="text" value="${ul}" readonly style="color:var(--text4)"/></div>
      <div><label>Min</label><input type="number" value="${v.min}" oninput="window.S.editorVars[${i}].min=this.value"/></div>
      <div><label>Max</label><input type="number" value="${v.max}" oninput="window.S.editorVars[${i}].max=this.value"/></div>
...
function renderVarInsertChips(){
  ...
  window.S.editorVars.forEach(v=>{
    const btn=document.createElement('button');btn.className='var-chip-btn';btn.textContent=`{${v.name}}`;
    btn.onclick=()=>{const ta=document.getElementById('e-question');const s=ta.selectionStart,e=ta.selectionEnd,ins=`{${v.name}}`;ta.value=ta.value.slice(0,s)+ins+ta.value.slice(e);ta.focus();ta.selectionStart=ta.selectionEnd=s+ins.length;};
    wrap.appendChild(btn);
  });
  if(!window.S.editorVars.length){const n=document.createElement('span');n.style.cssText='font-size:11px;color:var(--text4)';n.textContent='Add variables to see insert buttons.';wrap.appendChild(n);}
}
...
function previewFormula(){
  ...
  try{const fn=new Function(...Object.keys(vals),`return (${formula})`);const res=fn(...Object.values(vals));
    prev.textContent=`Preview: ${window.S.editorVars.map(v=>`${v.name}=${vals[v.name]}`).join(', ')||'no vars'} → ${rnd(res,4)} ${document.getElementById('e-unit').value}`;}
  catch(e){prev.textContent=`⚠ ${e.message}`;}
}

function handleImg(e){
  const file=e.target.files[0];if(!file)return;
  const reader=new FileReader();reader.onload=ev=>{window.S.editorImg=ev.target.result;document.getElementById('img-preview-wrap').innerHTML=`<img src="${ev.target.result}" class="img-thumb" alt="Circuit"/>`;};
...
function saveProblem(){
  ...
  const prob={id:window.S.editingId||`prob-${Date.now()}`,title,topic:document.getElementById('e-topic').value.trim(),
    question,formula,vars:window.S.editorVars.map(v=>({...v})),unit:document.getElementById('e-unit').value,
    tol:document.getElementById('e-tol').value,defaultPts:parseInt(document.getElementById('e-pts').value)||10,
...
function loadProbToForm(prob){
  ...
  document.getElementById('e-title').value=prob.title||'';document.getElementById('e-topic').value=prob.topic||'';
