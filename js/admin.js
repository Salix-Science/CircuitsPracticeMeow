...
function renderGradeBtns(){
  ...
  window.DB.assignments.forEach(a=>{
    ...
    btn.innerHTML=`<i class="ti ti-chart-bar"></i> ${a.title}`;
    btn.onclick=()=>{
...
async function loadGradeView(assignId){
...
// ── Shell with tabs ───────────────────────────
function renderGradeShell(wrap, assignId){
  ...
  wrap.innerHTML = `
    <div class="grade-shell">
      <div class="grade-shell-head">
        <div>
          <div style="font-family:var(--font-display);font-size:15px;color:var(--accent2);letter-spacing:.06em">${assign.title}</div>
...
function switchGradeTab(tab, shellWrap){
...
// ── Tab 1: Grade table ────────────────────────
function renderGradeTable(el, assign, users){
...
function renderScoreDistribution(el, assign, users){
...
function renderProblemAccuracy(el, assign, users){
  if(!users.length){ el.innerHTML = emptyChart('No submissions yet.'); return; }
...
function renderSubmissionTimeline(el, assign, users){
...
// Vertical bar chart
function barChart(values, labels, maxVal, colorFn){
...
function horizontalBarChart(values, labels, maxVal, colors, annotations){
...
function renderAttemptLog(el, assign, users){
...
function renderAttemptRows(entries){
  ...
  return entries.map(e=>{
    ...
    return `<tr ${rowBg}>
      ...
      <td style="max-width:160px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${e.probTitle||e.probId}</td>
