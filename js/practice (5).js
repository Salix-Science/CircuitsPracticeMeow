...
function buildProbCardEl(p, isFolder) {
...
function checkMainAnswer() {
  ...
  if (u && (allOk || noMore) && !p._scored) {
    ...
    if (allOk) u.streak = (u.streak || 0) + 1; else u.streak = 0;
...
// ── Difficulty rating ─────────────────────────
function showDifficultyRating(probId) {
  // Don't show if already rated this session
  if (window._rated?.[probId]) return;
  const fb = document.getElementById('main-fb');
  if (!fb) return;
  const ratingHTML = `
    <div id="diff-rating-wrap" style="margin-top:10px;padding-top:10px;border-top:0.5px solid rgba(74,222,128,.2)">
      <div style="font-size:11px;color:var(--text3);margin-bottom:6px">How difficult was this problem?</div>
      <div style="display:flex;gap:4px;align-items:center">
        ${[1,2,3,4,5].map(n => `
          <button onclick="submitDifficultyRating('${probId}',${n})"
            style="background:none;border:none;cursor:pointer;font-size:22px;padding:2px;transition:transform .1s;color:var(--text4)"
            onmouseover="highlightStars(${n})"
            onmouseout="highlightStars(0)"
            id="star-${n}">★</button>`).join('')}
        <span style="font-size:10px;color:var(--text4);margin-left:6px">1=easy · 5=hard</span>
      </div>
    </div>`;
  fb.insertAdjacentHTML('afterend', ratingHTML);
}
...
async function submitDifficultyRating(probId, rating) {
...
async function updateProblemRating(probId, newRating) {
