...
function shuffleFolderProb() {
  ...
  if (authored) {
    // Reset attempt count so the student gets fresh attempts with the new numbers
...
function buildProbCardEl(p, isFolder) {
  ...
  const answerInputsHTML = (p.answers || [{id:'ans0',label:'Answer',answer:p.answer,unit:p.unit,tol:p.tol}]).map((a,ai) =>
    `<div class="multi-ans-row">
      ...
      <div class="answer-row" style="margin-bottom:6px">
        <input class="mono" type="number" step="any" placeholder="0.000"
          id="main-ans-${ai}" style="width:140px"/>
        <span style="font-size:13px;color:var(--text2);font-weight:500">${a.unit}</span>
      </div>
    </div>`
  ).join('');
...
function checkMainAnswer() {
  ...
  const results = answers.map((a, ai) => {
    ...
    return { ok, raw, label: a.label, answer: a.answer, unit: a.unit, tol: a.tol || 0.02, missing: false };
  });
  ...
  if (allOk) {
    ...
    fb.innerHTML = answers.length > 1
      ? '✓ All correct! ' + results.map(r=>`${r.label}: ${r.answer} ${r.unit}`).join(' · ')
      : `✓ Correct! Answer: ${results[0].answer} ${results[0].unit}`;
    fb.style.display = 'block';
