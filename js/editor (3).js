...
function previewFormula(){ previewAllFormulas(); }
...
function renderPmList(){
  ...
  window.DB.problems.forEach((p,i)=>{
    ...
    const ansCount=(p.answers||[p]).length; // number of answer boxes
    row.innerHTML=`<div class="pm-drag-handle"><i class="ti ti-grip-vertical"></i></div>
      <div class="pm-row-body">
        ...
        <div class="pm-row-meta">${p.topic||'—'} · ${p.vars.length} vars · ${ansCount} answer${ansCount!==1?'s':''} · ${maxAtt>0?maxAtt+' att.':'unlimited'}${!isEnabled?' · hidden':''}</div>
      </div>
    ...
    row.addEventListener('drop',e=>{
      ...
      window.DB.problems.splice(i,0,m);_dragSrcIdx=null;saveDB();renderPmList();
    });
...
async function removeProbFromFolder(folderId,probId){const folder=window.DB.folders.find(f=>f.id===folderId);if(!folder)return;const _remP=window.DB.problems.find(p=>p.id===probId);logAdminAction('remove_problem_from_folder',{folderId,folderName:folder.name,probId,probTitle:_remP?.title});folder.problemIds=folder.problemIds.filter(pid=>pid!==probId);await saveDB();renderFolderList();buildPracticeSidebar();}
async function deleteFolder(id){if(!confirm('Delete this folder?'))return;const _df=window.DB.folders.find(f=>f.id===id);logAdminAction('delete_folder',{id,name:_df?.name});window.DB.folders=window.DB.folders.filter(f=>f.id!==id);await Promise.all([deleteFromDB('folders',id),saveDB()]);renderFolderList();buildPracticeSidebar();}
