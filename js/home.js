...
function renderHomepage() {
      ...
      <div class="home-posts-grid">
        ${recentPosts.map(p => {
          ...
          return `<div class="home-post-card" onclick="showView('blog');setTimeout(()=>openBlogPost('${p.id}'),50)">
            <div class="home-post-meta">${catPill(p.category)}<span class="blog-card-date">${date}</span></div>
            <div class="home-post-title">${p.title}</div>
            ${p.excerpt ? `<div class="home-post-excerpt">${p.excerpt}</div>` : ''}
          </div>`;
        }).join('')}
      </div>
    </div>` : '';
      ...
      ${openAssignments.map(a => {
        ...
        return `<div class="home-assign-row" onclick="showView('assignments')">
          <div class="home-assign-info">
            <div class="home-assign-name">${a.title}</div>
  ...
  wrap.innerHTML = `
    <div class="home-outer">
      <div class="home-hero">
        <div class="home-hero-title">Welcome back, ${window.S.user}</div>
