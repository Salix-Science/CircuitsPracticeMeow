...
async function sendBulkNotification(type, subject, message) {
  ...
  const recipients = await window.getSubscribedEmails(type);
...
window.renderAdminNotifPanel = async function renderAdminNotifPanel() {
  ...
  // Fetch all users fresh — window.DB.users only contains the current admin
  let allUsers = [];
  try {
    allUsers = await window._fetchAllUsers();
  } catch(e) {
    wrap.innerHTML = `<div style="color:var(--red);font-size:12px">Error loading users: ${e.message}</div>`;
    return;
  }
