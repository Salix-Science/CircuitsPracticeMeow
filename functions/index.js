...
exports.submitAssignment = onCall({ enforceAppCheck: false, cors: true }, async (request) => {
  ...
  if (typeof assignId !== 'string' || !assignId || typeof probId !== 'string' || !probId ||
  ...
  if (existingSub) {
    return { allOk: existingSub.correct, locked: true,
      details: (existingSub.details || []).map(d => ({ label: d.label, ok: d.ok, unit: d.unit })),
      attemptsUsed: existingAttempts, attemptsMax: maxAtt };
  }
