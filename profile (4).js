...
window.saveNotifPrefs = async function saveNotifPrefs() {
  ...
  if (email && !isValidEmail(email)) {
    if (ok) { ok.textContent = '⚠ Enter a valid email address.'; ok.classList.remove('hidden'); }
    return;
  }

  const u = window.DB.users[window.S.user];
  if (!u) {
    if (ok) { ok.textContent = '⚠ Not signed in — try refreshing.'; ok.classList.remove('hidden'); }
    return;
  }
  ...
  try {
    ...
    if (ok) {
      ok.textContent = `⚠ Save failed: ${e.message}`;
      ok.classList.remove('hidden');
    }
  }
}
...
window.getSubscribedEmails = async function(type) {
  const allUsers = await window._fetchAllUsers();
  return allUsers
